...
- **`Shift+Insert` e `Ctrl+Shift+Insert` agora colam** (convenção
  clássica de terminal, igual `Ctrl+V`) — nos 3 lugares onde colar faz
  sentido: editor principal, prompts (busca, salvar como, etc.) e
  modo de renomear (`F2`). Confirmei via ferramenta de diagnóstico de
  teclas que as duas chegam como a mesma tecla `Insert` com
  modificador diferente (`Shift` sozinho vs `Shift+Ctrl`), então as
  duas caem no mesmo tratamento. `Insert` **sem** modificador continua
  alternando o modo overwrite normalmente, sem regressão (testado).
  No modo de renomear não existia colar nenhum antes (nem `Ctrl+V`) —
  adicionado do zero, junto.

## v0.42.0

- **Corrigido: `Ctrl+V` não colava dentro de nenhum prompt** (busca,
  "Salvar como", ir pra linha, etc.) — só funcionava no editor
  principal. `handlePromptKey` simplesmente não tinha nenhum caso pra
  essa tecla. Achado testando o cenário exato reportado: clique duplo
  pra copiar uma palavra, `Ctrl+W` pra abrir a busca, `Ctrl+V` não
  fazia nada. Corrigido, e testado o fluxo completo: copia com clique
  duplo → `Ctrl+W` → `Ctrl+V` cola a palavra certa → `Enter` acha a
  ocorrência certa no texto. Texto colado com quebra de linha (ex:
  várias linhas selecionadas e copiadas) vira espaço dentro do prompt,
  já que é um campo de uma linha só — testado colando 2 linhas
  inteiras, virou uma frase só sem quebrar o campo.

## v0.41.0

- **`F6` agora usa os arquivos de sintaxe do próprio `nano`
  (`/usr/share/nano/*.nanorc`) em vez de um arquivo customizado no
  home do usuário** — cada arquivo já sabe pra qual extensão/shebang
  ele serve (`sh.nanorc` para bash/sh, `c.nanorc` para C, `go.nanorc`
  para Go, ...), então o `goedit` escaneia o diretório inteiro
  (`ScanNanorcDir`) e acha o certo sozinho, igual o `nano` de
  verdade faz. Testado contra os 44 arquivos reais do sistema:
  confirmado que `.c`→`c`, `.go`→`go`, `.sh`→`sh`, `.py`→`python`,
  `.js`→`javascript` batem cada um com o bloco certo (não só "algo
  bateu"). Achei 2 arquivos (`debian.nanorc`, `objc.nanorc`) com uma
  regra de cor cada que usa parêntese não balanceado — tolerado pelo
  motor de regex do `glibc` que o nano usa, rejeitado pelo `RE2`
  (mais rígido) do Go; documentado como limitação conhecida, não
  afeta o resto do arquivo nem os exemplos citados (sh/c/go).
- **Estado do `F6` (ligado/desligado) agora persiste no
  `settings.json`** (`useCustomNanorc`), igual o perfil de teclado
  (`F9`) — já vem ligado automaticamente na próxima vez que abrir,
  sem precisar apertar `F6` de novo. Testado: confirmei que a
  renderização já sai diferente **na abertura** (bytes do terminal
  diferentes), sem nenhuma tecla apertada, quando estava ligado da
  sessão anterior.

## v0.40.0

- **`F6` alterna pra um esquema de cores de sintaxe customizado
  (`~/.config/goedit/custom.nanorc`), na hora, sem reabrir o
  arquivo.** Usa o mesmo formato de arquivo `.nanorc` do `nano` de
  verdade — implementado um parser completo (`internal/syntax/nanorc.go`):
  - Diretivas `syntax`/`header`/`magic`/`color`/`icolor`, com
    detecção por nome de arquivo E por shebang/conteúdo, igual o nano
  - Conversor de regex GNU→Go (RE2): `\<`/`\>` → `\b`, e correção de
    barra invertida literal dentro de colchetes — validado
    semanticamente (não só "compila"), lendo o código-fonte real do
    nano (`rcfile.c`) pra confirmar a regra exata de fechamento de
    aspas dentro da regex
  - Paleta completa: 8 cores básicas + `bright`/`light`, as 25 cores
    estendidas do nano 7.x (mesmos índices de paleta 256 cores),
    `#rgb` hex de 3 dígitos, `fg,bg`
  - Se o arquivo não existir ou não bater com o arquivo aberto, avisa
    na barra de status e cai pro realce padrão (chroma) automaticamente
  Testado com um `.nanorc` real enviado pelo usuário: todas as 13
  regras carregam sem erro, cores mudam de verdade (confirmado nos
  bytes brutos do terminal), liga/desliga funciona em sequência,
  arquivo sem `custom.nanorc` avisa sem travar, arquivo que não bate
  com nenhuma regra cai pro chroma sem quebrar. Suíte de testes de
  regressão própria (`internal/syntax/nanorc_test.go`) usando esse
  arquivo real como dado de teste fixo do projeto.

## v0.39.0

- **Abertura ~3.5x mais rápida** (~30ms → ~8ms, medido via pty). Causa
  raiz identificada por perfilagem sistemática: a biblioteca `tcell`
  constrói, antes até do `main()` rodar, uma tabela de 557KB com a
  largura de exibição de cada caractere Unicode (`runewidth.CreateLUT`)
  — custando ~19-21ms de inicialização sozinha, medido isoladamente e
  confirmado como a maior fatia da diferença de "peso" percebida em
  relação ao `nano`.
  Corrigido com um patch mínimo numa cópia local do `tcell`
  (`third_party/tcell/`, referenciada via `replace` no `go.mod`) que
  desativa essa tabela por padrão — sem precisar de variável de
  ambiente nenhuma. Quem quiser o comportamento original pode setar
  `TCELL_FORCE_LUT=1`.
  Testado: exibição de emoji e caracteres CJK (chinês) continua
  correta e alinhada sem a tabela; impacto na digitação normal é de
  ~0.1ms a mais por tecla (imperceptível); build limpo do zero
  confirmado funcionando; `go vet`/`gofmt` limpos; desfazer/salvar
  sem regressão.

## v0.38.0

- **Corrigido: arquivos terminados em quebra de linha (`\n`, a
  convenção normal de arquivo de texto) não mostravam a última linha
  em branco**, diferente de `nano`, `mousepad`, `vim`, VSCode e
  praticamente todo editor de texto de verdade. `Ctrl+End` não ia pra
  lá, contagem de linhas ficava 1 a menos do que devia.
  Corrigido lendo o código-fonte real do `nano` (`files.c`, função
  `read_file`) pra confirmar o comportamento exato: quando o arquivo
  termina em `\n`, isso implica uma linha final vazia que já faz
  parte da estrutura real do arquivo — não é algo "criado" ao
  navegar até lá. A leitura do goedit agora reproduz exatamente essa
  lógica (trocado `bufio.Scanner` por leitura direta + separação por
  `\n`, que naturalmente conta essa linha extra).
  **Achado e corrigido no processo**: um bug real que essa mudança
  expôs — sobrava uma escrita de quebra de linha extra no `Save()`
  (resquício de antes, que compensava a leitura antiga perder essa
  informação), duplicando a quebra final ao salvar. Removido.
  Testado com hash MD5 byte a byte: arquivo terminado em `\n` agora
  mostra a linha extra corretamente (`Ctrl+End` vai pra ela, igual o
  `nano` reportou via `Ctrl+C`/Cur Pos), e salvar sem digitar nada
  reproduz o arquivo **idêntico**, hash a hash. Arquivo sem quebra no
  fim, arquivo vazio, arquivo novo e desfazer — todos confirmados sem
  regressão.

## v0.37.0

- **Posição do cursor restaurada ao reabrir um arquivo agora
  centraliza a linha na tela**, em vez de rolar só o mínimo necessário
  (que deixava colado na borda de cima/baixo, igual o problema já
  corrigido antes na busca). Como isso acontece antes da primeira
  renderização (o layout ainda não existe nesse ponto), a altura útil
  é calculada direto do tamanho real da tela.
  Testado: meio do arquivo (centraliza), perto do início e do fim
  (não tenta rolar além dos limites), arquivo pequeno que cabe
  inteiro na tela (não rola à toa), arquivo sem posição salva (abre
  normal na linha 1), e nos 3 perfis de teclado — todos idênticos.
