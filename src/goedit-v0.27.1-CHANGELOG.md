...
- Corrigido: sobrava uma linha em branco embaixo da barra de status —
  ela era reservada permanentemente pra prompts/mensagens, mesmo
  quando não tinha nada pra mostrar. Agora só é reservada quando tem
  algo de verdade (prompt aberto, renomear ativo, ou mensagem de
  status); sem isso, o editor usa a tela inteira e a barra de status
  desce pra última linha.
- No processo, achei que a mensagem de status (ex: "salvo") ficava
  travada na tela até o próximo uso da busca — único lugar que
  limpava ela antes. Agora limpa sozinha na próxima tecla ou clique,
  senão a correção acima não teria efeito prático no dia a dia (a
  linha ficaria "presa" reservada desde o primeiro `Ctrl+S`).
