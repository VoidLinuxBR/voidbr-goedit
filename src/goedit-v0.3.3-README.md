...
| `F2` | Renomear ao vivo — destaca todas as ocorrências exatas da palavra sob o cursor (a seleção, se houver, não muda o alvo — igual o VS Code com LSP) e atualiza o texto conforme você digita. O cursor do campo começa na mesma posição relativa que estava na palavra (início/meio/fim). `Enter` confirma, `Esc` cancela, `←`/`→`/`Home`/`End` navegam, `Insert` alterna inserir/sobrescrever |
